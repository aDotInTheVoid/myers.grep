...
char *argv[];
{
  ...
  int i;
  int len, alpha;
  static char *cset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123"
                      "456789!@#$%^&*()";

  progname = argv[0];

  if (argc < 2)
    error("Usage is '%s <pat> [ dif ] [ file ]'", progname);
  ...
  if (argc >= 4) {
    ...
    if (ifile == -1)
      error("Can't open file %s", argv[3]);
  } else
    ifile = 1;
